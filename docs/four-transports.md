# The four avenues between `act_web_server` and `act_api_server`

`anticaptrad` follows the fleet-wide contract in
...
|---|------|------|-------------------|
| 1 | `direct_read` | `act_web_server` → Postgres, via `act-lib-core` | a read on the page's critical path |
| 2 | `http` | `act_web_server` → `act_api_server`, stateless | the default, and anything that writes |
| 3 | `tcp` | `act_web_server` ⇄ `act_api_server`, held open | chatty internal calls where the handshake dominates |
| 4 | `jet_stream` | `act_web_server` → NATS → `act_api_server` | work the caller does not need to watch finish |

All four carry the same envelope and land on the same handler, so a
`act-e2e` suite can drive one operation four ways and assert the answers
...
|---|---|
| Slug | `act` |
| Environment prefix | `ACT_` |
| Request subject | `act.api.operation.v1` |
| Result subject | `act.api.operation-result.v1` |
| Durable consumer | `act-api-server-v1` |
...
|----------|--------|--------------|
| `ACT_READONLY_DATABASE_URL` | 1 | no direct reads |
| `ACT_API_URL` | 2 | no stateless HTTP |
| `ACT_API_MTLS_ADDR` | 3 | no stateful TCP |
| `ACT_API_TLS_SERVER_NAME` | 3 | plaintext to a TLS-terminating mesh |
| `ACT_WEB_CLIENT_CERT_FILE` | 3 | ” |
| `ACT_WEB_CLIENT_KEY_FILE` | 3 | ” |
| `ACT_API_CA_FILE` | 3 | ” |
| `ACT_NATS_URL` | 4 | no asynchronous avenue |

A missing variable disables its avenue. A variable that is present and unsafe
fails startup: a plaintext `ACT_API_URL` off loopback, a
`ACT_NATS_URL` that is not `tls://`, or mutual TLS with two of its
four files set.

In-cluster, `ACT_NATS_URL` points at the messaging service defined in
...
**In `act_web_server`** — implement `DirectReader` over `act-lib-core`:
...
impl DirectReader<Operation, Projection> for LibCoreReader {
        ...
        -> Result<Projection, TransportError>
    {
        ...
        // not an access check.
        // Then build the query with act-lib-core's query builders.
    }
...
**In `act_api_server`** — implement `OperationHandler` once, and hand the same
...
**Avenue 1 is read-only twice over.** `act-lib-core` gates its write surface
behind a `read-write` cargo feature and migrations behind `migrate`. Depend on
it from `act_web_server` as:

```toml
act-lib-core = { git = "…", default-features = false, features = ["read-only"] }
```

so a mutating call is a *compile error*, not a code-review miss. Separately,
`ACT_READONLY_DATABASE_URL` must name a Postgres role without
`INSERT`, `UPDATE`, `DELETE` or DDL. A mistake has to defeat both. Migrations
belong to `act-lib-core` and `declarative-migrations`, never to a web server.
