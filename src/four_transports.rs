#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `act_api_server`.
//!
...
//! to anticaptrad — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `act-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `act_api_server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `act_api_server` cluster |
...
//! `act-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `ACT_API_URL`, `ACT_NATS_URL`, and the rest; see the
